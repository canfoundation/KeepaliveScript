...
INSTANCE_ID=i-004a3e8fa4d35213f
TYPE_NODE=SLAVE
...
function notify()
{
    MESSAGE=$1
    echo $MESSAGE
    [[ ! -z $SLACK_WEBHOOK ]] && curl -s -X POST --data-urlencode "payload={\"channel\": \"#can-testnet\", \"username\": \"MumbaiBP-Backup\", \"text\": \"$MESSAGE\", \"icon_emoji\": \":ghost:\"}" $SLACK_WEBHOOK > /dev/null
}
